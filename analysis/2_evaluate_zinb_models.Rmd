...
```
```{r}
setwd("emomis-discussion-analysis/analysis/")
dst <- "./engagement/"
```


```{r}
src <- "emomis-discussion-analysis/data/"
df_regression <- read_csv(file.path(src, "matched_starters.csv")) 
...
```{r}
like_zinb <- readRDS("./engagement/zinb/like_count_zinb_model.rds")
quote_zinb <- readRDS("./engagement/zinb/quote_count_zinb_model.rds")
reply_zinb <- readRDS("./engagement/zinb/reply_count_zinb_model.rds")
retweet_zinb <- readRDS("./engagement/zinb/retweet_count_zinb_model.rds")
...
```{r}
like_nb <- readRDS("./engagement/nb/like_count_nb_model.rds")
quote_nb <- readRDS("./engagement/nb/quote_count_nb_model.rds")
reply_nb <- readRDS("./engagement/nb/reply_count_nb_model.rds")
retweet_nb <- readRDS("./engagement/nb/retweet_count_nb_model.rds")
...
## Compare model fit
```{r}
like_poisson <- read.csv("./engagement/poisson/like_count_pois_fit.csv")
quote_poisson <- read.csv("./engagement/poisson/quote_count_pois_fit.csv")
reply_poisson <- read.csv("./engagement/poisson/reply_count_pois_fit.csv")
retweet_poisson <- read.csv("./engagement/poisson/retweet_count_pois_fit.csv")
...
```{r}
create_zinb_table <- function(model, dv) {
  # load model summary csv
  model_summary <- read.csv(paste0("./zinb/", dv, "_zinb_summary.csv"))
  ...
  rating_zero <- model_summary %>%
    ...
    dplyr:::select(estimate, std_error, p_value, `95%CI`, OR) %>%
    rename(
      Estimate = estimate,
      `SE` = std_error,
      `p` = p_value
  ...
  rating_count <- model_summary %>%
    ...
    dplyr:::select(estimate, std_error, p_value, `95%CI`, IRR, PC) %>%
    rename(
      Estimate = estimate,
      `SE` = std_error, 
      `p` = p_value
...
like_zinb_table <- create_zinb_table(like_zinb, "like_count")
quote_zinb_table <- create_zinb_table(quote_zinb, "quote_count")
reply_zinb_table <- create_zinb_table(reply_zinb, "reply_count")
retweet_zinb_table <- create_zinb_table(retweet_zinb, "retweet_count")
...
zinb_table %>%
  kable("latex", 
        ...
        linesep = "",
        align = c("l", rep("r", 8))) %>%
  save_kable(file.path(dst, "zinb_table.tex"))
...
create_nb_table <- function(dv) {
  # load model summary csv
  model_summary <- read.csv(paste0("./nb/", dv, "_nb_summary.csv"))
  ...
  rating_row <- model_summary %>%
    ...
    dplyr:::select(estimate, std.error, p.value, `95%CI`, IRR, PC) %>%
    rename(
      Estimate = estimate,
      `SE` = std.error,
      `p` = p.value
...
nb_table %>%
  kable("latex", 
        ...
        linesep = "",
        align = c("l", rep("r", 6))) %>%
  save_kable(file.path(dst, "nb_table.tex"))
...
```{r}
like_nb_dh <- readRDS("./engagement/nb/like_count_nb_dharma.rds")
quote_nb_dh <- readRDS("./engagement/nb/quote_count_nb_dharma.rds")
reply_nb_dh <- readRDS("./engagement/nb/reply_count_nb_dharma.rds")
retweet_nb_dh <- readRDS("./engagement/nb/retweet_count_nb_dharma.rds")
```
...
model_fit %>%
  kable("latex", 
        ...
        linesep = "",
        align = c("l", "l", rep("r", 7))) %>%
  save_kable(file.path(dst, "all_models_fit.tex"))
...
```{r}
like_zinb_dh <- readRDS("./engagement/zinb/like_count_zinb_dharma.rds")
quote_zinb_dh <- readRDS("./engagement/zinb/quote_count_zinb_dharma.rds")
reply_zinb_dh <- readRDS("./engagement/zinb/reply_count_zinb_dharma.rds")
retweet_zinb_dh <- readRDS("./engagement/zinb/retweet_count_zinb_dharma.rds")
...
pdf(paste0(dst, "nb_dharma_plots.pdf"), 
...
pdf(paste0(dst, "zinb_dharma_plots.pdf"), 
            width = 6.5, height = 6.5)
