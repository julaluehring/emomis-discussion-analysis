...
```
```{r}
setwd("/home/luehring/german-newsguard-emotions/analysis/engagement")
dst <- "./engagement/"
```
...
write.csv(means_df, "avg_predictions_mfx.csv", row.names = FALSE)
means_df
```
...
mfx_df <- means_df %>%
  ...
  dplyr::select(DV, Estimate, SE, z, p, `95%CI`, PC)
...
mfx_df %>%
  ...
  kable("latex", 
        ...
        linesep = "",
        align = c("l", rep("r", 7))) %>%
  save_kable(file.path(dst, "mfx_table.tex"))
```
